...
exedir=${CMSSW_BASE}/src/CheckUncertainties
...
g++ -I $ROOTSYS/include -I $ROOFITSYS/include -I ${melaincdir} -I ${cmsswincdir} checkUncertainties.cc `root-config --glibs` `root-config --libs` `root-config --cflags` -L $ROOFITSYS/lib  -lRooFit -lRooFitCore -L ${melalibdir} -L ${cmsswlibdir}  -lZZMatrixElementMELA   -lZZMatrixElementMEMCalculators -lKaMuCaCalibration -lCondFormatsJetMETObjects -l JetMETCorrectionsModules  -o checkUncertainties
